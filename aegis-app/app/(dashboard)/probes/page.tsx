...
import {
  Cpu, Plus, Copy, Check, Loader2, AlertCircle,
  Terminal, Wifi, WifiOff, Trash2,
} from "lucide-react";
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm text-muted-foreground/60 text-center">
              Create a probe and run it on your Linux server
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {probes.map((probe) => (
            <Card
              key={probe.id}
              className={cn("transition-all", probe.status === "online" && "border-emerald-500/20")}
            >
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  <span className="font-mono truncate flex-1">{probe.probeId}</span>
                  {/* Delete button */}
                  <button
                    onClick={() => setConfirmDelete(probe)}
              ...
              <CardContent className="space-y-2">
                ...
                {probe.version && (
                  <p className="text-xs text-muted-foreground">v{probe.version}</p>
                )}
              </CardContent>
            </Card>
          ))}
        ...
        <DialogContent>
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              setError("");
              createProbe.mutate({ name: probeName });
            }}
            className="space-y-4"
          >
            {error && (
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Copy your API key and install command. The key won&apos;t be shown again.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => copyText(newKey, "key")}>
                  {copied === "key" ? (
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative bg-muted rounded-lg p-3">
                <pre className="text-xs font-mono text-muted-foreground overflow-x-auto whitespace-pre-wrap pr-10">
                  {installCmd}
                </pre>
                <Button
                  variant="outline"
            ...
            <Button className="w-full" onClick={() => setShowKey(false)}>
              Done
            </Button>
          </div>
        </DialogContent>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-mono font-semibold">{confirmDelete?.probeId}</span>.
              All associated metrics will be deleted. This cannot be undone.
            </DialogDescription>
          </DialogHeader>
